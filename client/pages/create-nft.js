...
import {CHAIN_ID, contractAddress, PINATA_KEY, PINATA_SECRET} from '../config';
...
export default function createNFT(){
    ...
    async function listNFTForSale(){
        ...
        let transaction = await contract.createToken('https://gateway.moralisipfs.com/ipfs/' + url.split('/ipfs/')[1],price,{value:listingPrice});
    ...
    return (
        <div className='flex justify-center'>
            ...
            <div className='w-1/2 flex flex-col'>
                ...
                <input type='file' name='Asset'  onChange={imageUpload} className='font-bold mt-4 bg-blue-700 text-white rounded p-4 shadow-lg'/>

                {
                    fileUrl && (
                        <button onClick={listNFTForSale} className='font-bold mt-4 bg-pink-500 text-white rounded p-4 shadow-lg'>
                            {loadingState == 'not-loading' ? 'Create NFT' : 'Working on it'}
                        </button>
                    )
                }
            </div>
