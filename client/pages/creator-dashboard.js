...
export default function CreatorDashboard(){
  ...
  if(loadingState == 'loaded' && !nfts.length) return (
      <div className='flex justify-center flex-col'>
          <h1 className='px-20 py-10 text-xl text-center'>No NFTs listed by you</h1>
              ...
              nfts.map((nft,i)=>(
                  <div key={i} className='shadow overflow-hidden mx-5 my-5'>
                      ...
                      <div className='p-4 bg-black'>
                          ...
                          <button className='w-full bg-red-500 text-white font-bold py-2 px-12 rounded' onClick={()=>cancelListing(nft.tokenId,nft.price)}>Resell NFT</button>
                      </div>
                  </div>              ))
            }
        </div>
